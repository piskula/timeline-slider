...
\thesissetup{
    ...
    keywords      = {Angular, KYPO, Liferay, časová os, SVG, D3.js, TypeScript, W3C, dynamické prostredie, range slider},
    TeXkeywords   = {Angular, KYPO, Liferay, časová os, SVG, D3.js, TypeScript, W3C, dynamické prostredie, range slider},
    abstract      = {Cieľom tejto práce je modifikácia existujúceho modulu časovej osy, ktorý sa používa v portáli Kybernetického polygónu Masarykovej univerzity a jeho implementácia vo frameworku Angular. Súčasťou práce je funkčný portlet, ktorý je možné integrovať do Liferay portálu.},
    thanks        = {Rád by som sa poďakoval vedúcemu práce RNDr. Radkovi Ošlejškovi, Ph.D. za jeho čas a pomoc pri tvorbe tejto práce.},
    %bib           = prace.bib,
}
...
\item Restové rozhranie bude modulu poskytovať zoznam povolených časových známok, pričom môžeme predpokladať, že rozostup medzi nimi (v práci ďalej označovaný ako \textit{step}) bude vždy zhodný.
...
% V rámci nefunkčných požiadaviek bol pre implementáciu zvolený framework Angular od Google, ktorého verzia v čase tvorby tejto práce bola \angular_version . Tento framework bol v prostredí KYPO zvolený globálne a preto je táto voľba logická...TODO
...
\item užívateľ má pred sebou grafickú informáciu, kde približne sa svojou hodnotou nachádza v~rámci povoleného rozsahu, čo je výhodou hlavne pri veľkých číslach.
\end{enumerate}
...
Naproti tomu som sa však nevyhol viacerým slepým vývojovým vetvám. Najväčšie problémy mi spôsobovali externé knižnice, ktorými som sa snažil si implementáciu uľahčiť. Následne mi všetko potrebné poskytla knižnica D3.js, ktorá sa stará o zobrazovanie vektorovej grafiky v prehliadačoch.

Svoju implementáciu som opísal v \ref{implementation}. kapitole, kde som uviedol z~môjho pohľadu najdôležitejšie implementačné detaily. Tie sa týkali hlavne architektúry a rozloženia komponent, ale taktiež odôvodnenia použitia jednotlivých grafických prvkov a ich umiestnenia. V~rámci práce som využil metodiku \textit{Continuous Integration} (CI - priebežná integrácia výsledkov), pomocou ktorej som automaticky vytvoril plne funkčnú klikateľnú aplikáciu pri každej zmene kódu v Git repozitári projektu. Toto demo mal zadávateľ (vedúci práce) stále k dispozícii, aby na základe jeho pripomienok bolo možné upraviť funkcionalitu.

Pre overenie funkčnosti som výsledný portlet úspešne nasadil na~lokálne bežiaci portálový server Liferay. Pri transformácii Angular modulu na~archív WAR mi veľmi pomohol projekt Davida Berana (ktorý medzičasom vznikol v~prostredí KYPO). Musel som ho však na~viacerých miestach upraviť z dôvodu použitia novšej verzie Angularu (\angular_version ). Po nasadení do portálu bol modul plne funkčný, objavili sa len problémy s dedičnosťou štýlov, ktoré čiastočne výsledný vzhľad deformovali.

Keďže niektoré stavy, ktoré si drží modul (stav zámkov, minima, maxima, zvoleného intervalu) sa môžu navzájom ovplyvňovať, vytvoril som jednotkové testy, ktoré pokrývajú netriviálnu logiku implementovanú v~službe ShareTimeService. Tieto udalosti môžu byť vyvolané buď príchodom nových dát (zmenou možného ohraničenia), alebo činnosťou užívateľa.

Vhodným pokračovaním tejto práce by bolo nasadenie portletu do produkčnej verzie Liferay portálu a intenzívne užívateľské testovanie, ktoré by mohlo odhaliť slabiny dizajnového riešenia. Takéto testovanie vie priniesť veľmi cenný pohľad koncového používateľa, ktoré ja v~pozícii vývojára nemusím postrehnúť a ktoré som vykonával len v~neformálnej rovine na~používateľoch mimo KYPO.
