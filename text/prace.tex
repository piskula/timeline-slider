...
\chapter{Požiadavky na modul}
Cieľom tejto práce je vytvoriť modul vo frameworku Angular, ktorý bude nasaditeľný ako samostatný portlet do Liferay portálu a prostredníctvom ktorého bude užívateľ schopný zvoliť si podmnožinu časového ohraničenia. Modul tak umožní užívateľovi spresniť (zúžiť) časové okno. Vstupom tohto modulu bude
\begin{itemize}
\item informácia o najmenšej uvažovanej časovej jednotke (step) - REST API
\item dolné a horné časové ohraničenie (min, max) - REST API
\item verejná IP adresa, na ktorú budú prebiehať volania REST API - konfigurácia
\end{itemize}
...
\begin{itemize}
\item modul si zistí možné časové ohraničenie volaním verejného aplikačného rozhrania (API) cez REST službu
...
\begin{center}
  \begin{tabular}{ | c | c || p{6.5cm} | }
    ...
    zamknutý & odomknutý & zakázaný stav - ľavý zámok môže byť aktivovaný len ak je aktivovaný zároveň s pravým \\ \hline
